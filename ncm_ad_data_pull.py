...
parser.add_argument(
    "--lag_days", type=int, default=2, help="Number of lag days to process"
)
        ...
        for file in files:
            ...
            if model_name in df_manifest.columns:
                existing_prediction_time = (
                    df_manifest[model_name].dropna().index.tolist()
                )
                existing_prediction_time = [
            ...
            try:
                if date_name not in existing_prediction_time:
                    ...
                    if response.status_code == 200:
                        ...
                        gcs_path = os.path.join(GCS_PATH, model_name, idate, filename)
                            ...
                            db_con.logging(
                                {
                                    "script": SCRIPT_NAME,
                                    "log_type": "info",
                                    "message": f"Data downloaded for {model_name}, {date_name}",
                                }
                            )
                        ...
                        if os.path.exists(zip_path) and local_size == remote_size:
                            db_con.logging(
                                {
                                    "script": SCRIPT_NAME,
                                    "log_type": "info",
                                    "message": f"Data avaialable for {model_name}, {date_name}",
                                }
                            )
                            df_out = utils.extract_ncm_ad(
                                fname=zip_path,
                                dest=ncm_temp_data,
                                df_stn=df_static,
                                zone=model_name,
                            ...
                            df_out.to_csv(
                                os.path.join(
                                    csv_path, f"{idate}{icycle}_{model_name}.csv"
                                ),
                                index=False,
                            )
                            df_db = pd.DataFrame(
                                columns=db_columns["weather_table"]["columns"]
                            )
                            ...
                            db_con.logging(
                                {
                                    "script": SCRIPT_NAME,
                                    "log_type": "info",
                                    "message": f"Data extracted for {model_name}, {date_name}",
                                }
                            )
                ...
                db_con.logging(
                    {
                        "script": SCRIPT_NAME,
                        "log_type": "error",
                        "message": f"Data not available for {model_name}, {date_name}",
                    }
                )
