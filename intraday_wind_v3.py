...
for _, idf in df_static.iterrows():
    try:
        ...
        df_nwp = df_nwp[["forecast_time", "wind_speed", "wind_direction"]]
        df_nwp = df_nwp.set_index("forecast_time")
        df_nwp = df_nwp.resample("15min").interpolate(method="time")
        df_nwp = df_nwp.reset_index()
        ...
        df_nwp_fct = df_nwp_fct[["forecast_time", "wind_speed", "wind_direction"]]
        df_nwp_fct = df_nwp_fct.set_index("forecast_time")
        df_nwp_fct = df_nwp_fct.resample("15min").interpolate(method="time")
        ...
        df_nwp_fct["model_name"] = MODEL_NAME
        ...
        df_nwp_fct.to_csv(f"{fct_dir}/intraday_wind_{farm_name}_{date_now.strftime('%Y%m%d_%H%M')}.csv", index = False)
