...
class APICon:
    def __init__(self, base_url="http://127.0.0.1:5000", config_path=None):
        ...
        self.config_path = config_path or os.getenv(
            "WEATHER_CONFIG", "reinsight_config.yml"
        )
        try:
    ...
    def upload_static_data(self, df):
        ...
        try:
            ...
            if "regulation_bands" in df.columns:
                ...
                df["regulation_bands"] = df["regulation_bands"].apply(
    ...
    def fetch_weather_data(
        self, plant_name, model_name=None, start_date=None, end_date=None
    ):
...
class DBcon:
    ...
    def get_log_data(self, script, start_date, end_date):
        df_log = pd.read_sql(
            ...
            and created_at between '{start_date} 00:00:00' and '{end_date} 23:59:59' order by created_at",
            con=self.conn,
        )
        return df_log
...
def get_dsm_regulation(actual, forecast, avc_kw, regulation, ppa_rate=5.0):
    actual_kwh = actual / 4
    ...
    if regulation == "CTU-WIND":
        slab1_err = np.minimum(error_pct, 15)
        slab2_err = np.minimum(np.maximum(error_pct - 20, 0), 5)
        slab3_err = np.maximum(error_pct - 20, 0)
        slab2_energy = (slab2_err / 100) * avc_kwh
        slab3_energy = (slab3_err / 100) * avc_kwh
        total_dsm_penalty_rs = (slab2_energy * 0.1) + (slab3_energy * ppa_rate)

    elif regulation == "CTU-SOLAR":
        slab1_err = np.minimum(error_pct, 10)
        slab2_err = np.minimum(np.maximum(error_pct - 15, 0), 5)
        slab3_err = np.maximum(error_pct - 15, 0)
        slab2_energy = (slab2_err / 100) * avc_kwh
        slab3_energy = (slab3_err / 100) * avc_kwh
        total_dsm_penalty_rs = (slab2_energy * 0.1) + (slab3_energy * ppa_rate)
    ...
    return {
        ...
        "DSM_Impact_Rs_per_Actual_kWh": (
            penalty_sum / actual_sum if actual_sum > 0 else 0.0
        ),
        "Total_loss_percentage": (
            (penalty_sum / revenue_sum) * 100 if revenue_sum > 0 else 0.0
        ),
    }
...
def filter_forecast_by_regulation(dfp, regulation="CTU", lag_hours=2):
    ...
    if regulation in ["CTU-SOLAR", "CTU-WIND"]:
        lag_mask = (dfp["forecast_time"] - dfp["prediction_time"]) >= pd.Timedelta(
            hours=1, minutes=45
        )
        dfp = dfp[lag_mask]
        ...
        pred_minutes_from_midnight = (
            dfp["prediction_time"].dt.hour * 60 + dfp["prediction_time"].dt.minute
        )
        dfp_filter = dfp[(pred_minutes_from_midnight - 1350) % 90 == 0].copy()

        dfp_filter["block_start"] = dfp_filter["prediction_time"] + pd.Timedelta(
            minutes=90
        )
        ...
        dfp_filter = dfp_filter.drop(columns=["block_start", "block_end"]).sort_values(
            by="forecast_time"
        )
        ...
        dfp["prediction_time"] = (
            pd.to_datetime(dfp["prediction_time"], utc=True)
...
def assign_global_and_regional_grids(
    df, global_boxes, regional_boxes, lat_col="latitude", lon_col="longitude"
):
    """
    ...
    for box in global_boxes:
        mask_global = (
            ...
            & (df_out[lon_col] >= box["lon_min"])
            & (df_out[lon_col] <= box["lon_max"])
            & (df_out[lat_col] >= box["lat_min"])
            & (df_out[lat_col] <= box["lat_max"])
        )
...
def extract_ncm_ad(fname, dest, df_stn, zone):
    ...
    for region in range(1, 4):
        ...
        for _, idf in df_st.iterrows():
            ...
            if zone == "ncum_g":
                df_temp_speed = (
                    ds_speed.sel(lat=lat, lon=lon, method="nearest")
                    .to_dataframe()
                    .reset_index()
                )
                df_temp_surf = (
                    ds_surface.sel(lat=lat, lon=lon, method="nearest")
                    .to_dataframe()
                    .reset_index()
                )
                df_temp_surf["lev"] = 0
    ...
    df_nwp = df_nwp.rename(
        {"time": "forecast_time", "lev": "height", "dswrf": "ghi"}, axis=1
    )
    df_nwp["forecast_time"] = pd.to_datetime(df_nwp["forecast_time"])
    df_nwp["prediction_time"] = df_nwp["forecast_time"].min()
