...
def get_wind_and_cloud_forecast(
    latitudes: List[float], longitudes: List[float], model: str, forecast_days: int = 3
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    db_con.logging(
        {
            "script": SCRIPT_NAME,
            "log_type": "info",
            "message": f"Openmeteo data fetch script started",
        }
    )
    ...
    OPENMETEO_MODEL_MANIFEST = (
        "../data_lake/re_insights/manifest_files/openmeteo_model_manifest.csv"
    )
    ...
    for model in models:
        try:
                ...
                db_con.logging(
                    {
                        "script": model,
                        "log_type": "error",
                        "message": f"Openmeteo data fetch script failed for {model}: {e}",
                    }
                )
                continue
            ...
            df_target["prediction_time"] = df_target["prediction_time"].dt.tz_localize(
                "UTC"
            )
            df_target["forecast_time"] = df_target["forecast_time"].dt.tz_localize(
                "UTC"
            )
            ...
            gcs_path = os.path.join(
                GCS_PATH,
                prediction_time.strftime("%Y%m%d"),
                model,
            ...
            db_con.logging(
                {
                    "script": model,
                    "log_type": "success",
                    "message": f"Openmeteo data fetch script completed for {model}, prediction time: {prediction_time_str}",
                }
            )
...
db_con.logging(
    {
        "script": SCRIPT_NAME,
        "log_type": "info",
        "message": f"Openmeteo data fetch script completed",
    }
)
